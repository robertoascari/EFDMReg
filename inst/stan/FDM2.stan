...
// likelihood
	for (i in 1:N) {
		...
		for(j in 1:D){
		  temp2=temp2+p[j]*w_norm*fmin(1.0, mu[i,j]/p[j]);
		}
		
		for(j in 1:D){//j rappresenta r
			alpha[j] = aplus*((mu[i,j] - p[j]*w_norm*fmin(1.0, mu[i,j]/p[j]))/(1-temp2));
			tau[j] = aplus*(w_norm*fmin(1.0, mu[i,j]/p[j])/(1-w_norm*fmin(1.0, mu[i,j]/p[j])));
